...
| 3 | **Creative fatigue / issue** | Top-5 active ads ranked by spend, CTR week-over-week. Frequency is **UI-sourced** — not in the MCP metric surface; ask the user for it rather than inventing it. **Also check paused ads** — a top performer that was paused looks identical to fatigue in the aggregate. | Frequency > 4 (user-supplied), CTR decay ≥ 30% from peak, or ad status changed to PAUSED. |
| 4 | **Tracking break** | Attributed-conversions trend from the dynamic report (per-rule conversion metrics where they matter), 14 days before drop vs after, with clicks alongside. No MCP tool exposes fired/matched ratios — a break here can't be localized from reports alone, so a suspicious shift hands off to `diagnose-tracking`. | Attributed conversions down > 20% while clicks held steady. |
...
| 6 | **Creative restructure shock** | Cluster item-level changes by hour, from the change log (`get_campaign_history_report`) where it surfaces item-level changes — verify against real output; where it doesn't, ask the user what was restructured and when. If ≥ 3 distinct campaigns had item-level changes in the same hour AND a CPA / ROAS break followed within 48h, flag as restructure shock. | Coordinated pause-old + launch-new on the same day → 24-48h CPA break, 5-7 day recovery. |
...
- **Pause an underperforming item** — MCP write via `manage-campaigns` (`update_native_item` / `update_display_item`, `is_active=false`).
- **Block a site** — MCP write via `manage-campaigns` (merge into `publisher_targeting` EXCLUDE — full-replace semantics, so read-and-merge). Run the historical-top-N guard (in `knowledge/site-management.md`) before recommending.
- **Adjust daily budget** — MCP write via `manage-campaigns` (`update_campaign`, `daily_cap`). Per-change cap: 20%. Per cadence: every 2-3 days.
- **Add a Target CPA** — MCP write via `manage-campaigns` (`update_campaign`) — but **last-resort lever only** (see `knowledge/bidding.md`, including the bid-strategy-switch rules). Set within 10-20% of stable CPA after 3-4 days of delivery — never at launch, never aspirational.
- **Refresh creatives significantly** — MCP write via `manage-campaigns` (`create_native_item` / `create_display_item`). Recommended: 3 distinct titles + 3 unique images per campaign; 4–6 items per campaign for algorithm testing, never more than 10.
- **Tighten / broaden targeting** — MCP write via `manage-campaigns` (`update_campaign` targeting blocks — full-replace within a section, so read-and-merge).
- **Isolate a top performer** — MCP writes via `manage-campaigns` (`create_campaign` + item creates) containing only the winning item(s) at a fresh learning state.
