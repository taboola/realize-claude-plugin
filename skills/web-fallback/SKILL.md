...
allowed-tools: ["Read", "Bash", "AskUserQuestion", "WebSearch", "WebFetch"]
---
...
This skill closes that gap with one narrow move: look the topic up in Taboola's public advertiser help documentation, and answer with a *"this is what I found online"* framing. It is the last tier of the sourcing ladder in `os/guardrails.md` → *Sourcing*, and it never outranks anything above it.
...
- **The search summary defeats the path filter if you read it.** The tool synthesizes an answer across *all* hits before you ever see the URL list, so filtering links afterwards filters nothing you've already absorbed. In one live run the summary explained what a pixel is from a marketing page, recommended a **competitor's** browser extension for verification, and closed by suggesting the user check some other product's documentation — from a search that was correctly domain-restricted. The links are the deliverable of step 3; the prose is not.
