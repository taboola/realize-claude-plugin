...
- **Pass IDs verbatim downstream — and respect their underlying type.** `account_id`, `campaign_id`, `item_id`, and publisher `id` are **opaque strings** returned by the API; do not coerce to int. `audience_id`, `segment_id`, and lookalike `rule_id` are **integers** — pass them as numbers, not stringified. Conversion-rule ids split by context: **integer** inside payloads that reference a rule (`conversion_rules.rules: [{id}]` on campaign writes), but the `rule_id` *parameter* on `get_conversion_rules` / `update_conversion_rule` takes that same id **as a string**. In all cases, do not strip or re-case the value returned by the search / list tool.
